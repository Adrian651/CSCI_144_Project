...
// These global variables will be used at the conditions on which an atom
...
// The function a hydrogen atom executes.  Pass in the identifier of the
// atom.
...
void *h_atom( void *arg )
{
	...
	if( h_atoms >= 2 && o_atoms >= 1 )
	{
		...
		for( i = 0; i < total * 2 / 3; i++ )
			...
			{
				h[ i ] = 0;
				cout << "H atom " << id << " signalling H atom " << i << endl;
		...
		for( i = 0; i < total / 3; i++ )
			...
			{
				o[ i ] = 0;
				cout << "H atom " << id << " signalling O atom " << i << endl;
...
void *o_atom( void *arg )
{
	...
	if( h_atoms >= 2 && o_atoms >= 1 )
	{
		...
		for( i = 0; i < total * 2 / 3; i++ )
			...
			{
				h[ i ] = 0;
				cout << "O atom " << id << " signalling H atom " << i << endl;
				pthread_cond_signal( &cv_h );
				break;
			}
		...
		for( i = 0; i < total / 3; i++ )
			...
			{
				o[ i ] = 0;
				cout << "O atom " << id << " signalling O atom " << i << endl;
				pthread_cond_signal( &cv_o );
				break;
			}
	}
